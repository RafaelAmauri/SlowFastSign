...
def rankSimiliratyByFeatures(labeledFeaturesPath, unlabeledFeaturesPath):
    ...
    for file in os.listdir(labeledFeaturesPath):
        ...
        featuresLabeledSet[filePath] = np.load(filePath, allow_pickle=True).item()['features'].numpy()
    ...
    for file in os.listdir(unlabeledFeaturesPath):
        ...
        featuresUnlabeledSet[filePath] = np.load(filePath, allow_pickle=True).item()['features'].numpy()
    ...
    for nameUnlabeledFeature, unlabeledFeature in tqdm(featuresUnlabeledSet.items()):
