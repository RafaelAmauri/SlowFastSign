...
def alignmentVideoGeneratedGloss(model, processor, glossesByFullVideoPath: dict, nFrames:int, predictionsWorkDir: str, device: str) -> dict:
    ...
    for vPath, predictedGloss in loop:
        # X-Clip can only handle 8 frames or 16 frames at a time, so we need to choose the correct amount of keyframes to sample
        keyFrames = getKeyFramesFromFolder(vPath, nFrames)
